    ...
    SubShader {
        ...
        CGPROGRAM
            ...
            void Surface (Input IN, inout SurfaceOutput o) 
            {
                ...
                half  SpecularIntensity = DeGamma(o.Gloss *_SpecColor);
                ...
                half3 EyeReflection = normalize(reflect(-normalize(IN.viewDir), o.Normal));

                dp.x = saturate(dot(EyeReflection, RNM_BASIS[0]));
                dp.y = saturate(dot(EyeReflection, RNM_BASIS[1]));
                dp.z = saturate(dot(EyeReflection, RNM_BASIS[2]));
                dp *= dp;

                dp = dp / dot(dp, half3(1.0f, 1.0f, 1.0f));

                SpecularLight = dp.x * Irradiance0 +
                                dp.y * Irradiance1 +
                                dp.z * Irradiance2;
#endif
